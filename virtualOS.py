...
def netcdf2PCRobjCloneWithoutTime(ncFile,varName,
    ...
    if sameClone == False:
        ...
        try:
            ...
            cropData = f.variables[varName][0][yIdxSta:yIdxEnd,xIdxSta:xIdxEnd]
...
def regridData2FinerGrid(rescaleFac,coarse,MV=MV):
    if rescaleFac ==1:
        return coarse
    ...
    for r in range(0,nr):
        for c in range(0,nc):
            ...
            if window_extension > 0: 
                ...
                max_r = min(fac *(r+1) + window_extension, np.shape(fine)[0])
                max_c = min(fac *(c+1) + window_extension, np.shape(fine)[1])
...
def clean_tmp_dir(tmpDir):

    command = 'rm -r '+tmpDir+'/*'
    os.system(command)

def varName2String(variable_name, locals_input):            # NOT TESTED YET

    for v in locals_input:
        if id(v) == id(str(variable_name)): return (str(v))
